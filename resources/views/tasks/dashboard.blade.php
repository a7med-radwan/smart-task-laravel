<x-layout title="Focus - Dashboard">
    ...
    <section class="grid grid-cols-1 sm:grid-cols-3 gap-gutter-desktop mb-stack-lg animate-fade-in">
        <!-- Total Tasks -->
        <div class="bg-surface-container p-6 rounded-xl card-elevation border border-outline-variant flex items-center gap-4">
            ...
            <div>
                <p class="font-label-md text-label-md text-on-surface-variant uppercase tracking-wider text-[11px]">Total Tasks</p>
        ...
        <div class="bg-surface-container p-6 rounded-xl card-elevation border border-outline-variant flex items-center gap-4">
            ...
            <div>
                <p class="font-label-md text-label-md text-on-surface-variant uppercase tracking-wider text-[11px]">Completed Tasks</p>
        ...
        <div class="bg-surface-container p-6 rounded-xl card-elevation border border-outline-variant flex items-center gap-4">
            ...
            <div>
                <p class="font-label-md text-label-md text-on-surface-variant uppercase tracking-wider text-[11px]">Pending Tasks</p>
                <h3 class="font-headline-lg text-headline-lg font-bold text-on-surface mt-1">{{ $totalTasks - $completedTasks }}</h3>
            </div>
        </div>
    </section>
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-gutter-desktop">
        ...
        <section class="md:col-span-8 bg-surface-container p-stack-lg rounded-xl card-elevation border border-outline-variant animate-fade-in">
            <div class="flex justify-between items-center mb-stack-lg">
                ...
                <a href="{{ route('tasks.index') }}" class="text-primary font-label-md text-label-md hover:underline font-bold">View All</a>
            </div>
            <div class="space-y-3">
                @forelse ($upcomingTasks as $task)
                    <a href="{{ route('tasks.show', $task->id) }}"
                        ...
                        <div class="flex-1 min-w-0">
                            <h4 class="font-body-lg text-body-lg font-bold group-hover:text-primary transition-colors truncate">{{ $task->title }}</h4>
        ...
        <section class="md:col-span-4 space-y-gutter-desktop animate-fade-in">
            ...
            <div class="bg-surface-container p-stack-lg rounded-xl card-elevation border border-outline-variant">
                ...
                <div class="space-y-3">
                    <a href="{{ route('tasks.create') }}"
                        ...
                        <div class="w-8 h-8 rounded-full bg-primary-container/10 text-primary flex items-center justify-center shrink-0">
                    ...
                    <a href="{{ route('tasks.index') }}"
                        class="w-full flex items-center gap-3 p-3 rounded-lg border border-outline-variant hover:bg-surface-container-high transition-all">
                        <div class="w-8 h-8 rounded-full bg-secondary-container/20 text-secondary flex items-center justify-center shrink-0">
            ...
            <div class="rounded-xl h-48 relative overflow-hidden group border border-outline-variant card-elevation">
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent flex items-end p-6">
                    <p class="text-white font-label-md text-label-md italic">"Focus is the art of knowing what to ignore."</p>
                </div>
            </div>
        </section>
    </div>
