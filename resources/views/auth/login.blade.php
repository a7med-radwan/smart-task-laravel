...
<body class="text-on-surface min-h-screen flex items-center justify-center relative overflow-hidden px-4">
    ...
    <div class="w-full max-w-[460px] bg-white border border-outline-variant rounded-2xl shadow-xl p-8 md:p-10 relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <div class="relative group">
                    ...
                    <input class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-outline-variant focus:border-primary focus:ring-1 focus:ring-primary font-body-md text-body-md bg-white outline-none transition-all"
                        id="email" type="email" name="email" placeholder="alexander.w@focus.com" value="{{ old('email') }}" required autofocus />
                </div>
            </div>
            ...
            <div>
                <div class="flex justify-between items-center mb-1.5">
                    <label class="font-label-md text-label-md text-on-surface-variant uppercase tracking-wider text-[11px]" for="password">Password</label>
                    <a href="{{ route('password.request') }}" class="text-label-sm text-primary hover:underline font-semibold">Forgot password?</a>
                </div>
                <div class="relative group">
            ...
            <div class="flex items-center">
                ...
                <label class="ml-2 font-body-md text-body-md text-on-surface-variant cursor-pointer select-none" for="remember">Remember me on this device</label>
            </div>
        ...
        <div class="mt-8 pt-6 border-t border-outline-variant text-center">
            <p class="text-body-md text-on-surface-variant">Don't have an account? <a href="{{ route('register') }}" class="text-primary font-bold hover:underline">Create an account</a></p>
        </div>
    </div>
